...
// monkey-patch Function.prototype.bind since the event listener that calls keyInput2 uses it bound to the game instance,
// and both the bound function and game instance don't have global references.
// patching addEventListener could be better but I haven't tried it yet
(function() {
    const originalBind = Function.prototype.bind;

    Function.prototype.bind = function(context, ...args) {
        // Function.prototype.bind is usually called as a method of the function to bind
        const originalFn = this;

        function wrapper(...args) {
            // if it's called by an event listener, args[0] is the event object
            // context should be the game instance
            if (args[0]?.type === "keydown" && OneKF.enabled && context.focusState === 0 && context.play) {
                OneKF.handleInput(args[0], context);
            }
            return originalFn.call(context, ...args);
        };

        return originalBind.call(wrapper, context, ...args);
    };
})();
...
window.OneKF = class OneKF {
    ...
    // on Firefox the semicolon key is 59 instead of 186
...
// repeatedly execute until Game is defined
